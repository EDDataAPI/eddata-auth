...
async function rotateTokens (cmdrId) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Error response returned by Frontier API: ${errorData?.error || response.statusText}`)
  }
  ...
  if (!responsePayload.access_token || !responsePayload.token_type) {
...
function getCache (cmdrId, key) {
  try {
    ...
    // If the cache is older than CAPI_CACHE_MAX_AGE treat as a cache miss
    if (cacheAge > CAPI_CACHE_MAX_AGE) return undefined

    // Attempt to deserialize cached data into object (but return raw if that fails)
    try {
    ...
    console.error(`Error retrieving cache for '${key}': ${e.message}`)
    return undefined
  }
}
