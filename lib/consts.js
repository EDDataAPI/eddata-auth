...
const ARDENT_AUTH_LOCAL_PORT = Number(process.env?.ARDENT_AUTH_LOCAL_PORT ?? 3003)

// Data directory configuration
const defaultDataDir = path.join(__dirname, '../../ardent-data')
...
const ARDENT_BACKUP_DIR = process.env?.ARDENT_BACKUP_DIR ?? path.join(ARDENT_DATA_DIR, 'backup')
...
if (!process.env?.ARDENT_AUTH_JWT_SECRET) {
  console.warn('WARNING: ARDENT_AUTH_JWT_SECRET was not set, generating temporary secret (will change when server restarts)')
