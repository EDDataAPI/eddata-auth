...
// Secrets MUST be at least 64 bytes for security
...
if (!process.env?.EDDATA_SESSION_SECRET) {
  ...
  process.env.EDDATA_SESSION_SECRET = randomBytes(MIN_SECRET_BYTES).toString('hex')
}
const SESSION_SECRET = process.env.EDDATA_SESSION_SECRET
if (SESSION_SECRET.length < MIN_SECRET_BYTES) {
  console.error('ERROR: EDDATA_SESSION_SECRET is too short (must be at least 64 bytes)')
  process.exit(1)
}
...
if (!process.env?.EDDATA_AUTH_JWT_SECRET) {
  ...
  process.env.EDDATA_AUTH_JWT_SECRET = randomBytes(MIN_SECRET_BYTES).toString('hex')
}
const AUTH_JWT_SECRET = process.env.EDDATA_AUTH_JWT_SECRET
if (AUTH_JWT_SECRET.length < MIN_SECRET_BYTES) {
  console.error('ERROR: EDDATA_AUTH_JWT_SECRET is too short (must be at least 64 bytes)')
  process.exit(1)
}
...
const AUTH_CALLBACK_URL = `${EDDATA_AUTH_BASE_URL}/auth/callback`
...
const JWT_COOKIE_OPTIONS = {
  ...
  maxAge: MAX_JWT_AGE_SECONDS * 1000
}
